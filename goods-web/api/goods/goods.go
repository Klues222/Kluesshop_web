...
func List(ctx *gin.Context) {
	//商品列表
	//获取参数 最大价格最小价格 是否热门 是否为新 分类 分页 搜索关键词 品牌
...
func Detail(ctx *gin.Context) {
	id := ctx.Param("id")
	i, err := strconv.ParseInt(id, 10, 32) //string to int
	if err != nil {
		ctx.Status(http.StatusNotFound)
		return
	}
	value, err := global.GoodsSrvClient.GetGoodsDetail(context.Background(), &proto.GoodInfoRequest{
		Id: int32(i),
	})
	if err != nil {
		HandleGrpcErrorToHttp(err, ctx)
